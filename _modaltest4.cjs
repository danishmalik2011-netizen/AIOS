...
(async () => {
  await send('', 800);
  await send('/model'); await send(KEY.enter, 500); await send('', 300); // first enter - accepts from slash menu
  await send(KEY.enter, 500); await send('', 500); // second enter - submits
  const s = strip(out);
  console.log('=== MODEL PAGE ===');
  console.log(s.slice(-800));
  child.kill(); process.exit(0);
})();
setTimeout(() => { console.log('TIMEOUT'); console.log(strip(out).slice(-1000)); child.kill(); process.exit(1); }, 10000);