...
(async () => {
  await send('', 500);
  // Test model page directly
  await send('/model'); await send(KEY.enter, 500); await send('', 300);
  // Should open model page for default provider (ollama)
  await send(KEY.down, 200); await send(KEY.down, 200); // move down a bit
...
setTimeout(() => { console.log('TIMEOUT'); console.log(strip(out).slice(-500)); child.kill(); process.exit(1); }, 10000);